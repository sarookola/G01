	...
	hGLWnd(NULL), hGLDC(NULL), hGLRC(NULL), hOldGLRC(NULL)	
{
	...
	pGLScene = new Osgtest01();
	pGLScene->_viewer->realize();
}
...
void BMDOpenGLOutput::ResetFrame()
{
	...
	memset(pFrame, 0xf5, pDLVideoFrame->GetRowBytes() * uiFrameHeight);
}
...
bool BMDOpenGLOutput::Start()
{
	...
	{
		pDLDisplayModeIterator->Next(NULL);
	...
	//MessageBox(NULL,itoa( uiFrameWidth,NULL,0),"/mahan", MB_OK);
...
void BMDOpenGLOutput::RenderToDevice()
{
	...
	pGLScene->runframe(0, 0, uiFrameWidth, uiFrameHeight);
	//pGLScene->image2->readPixels(0, 0, uiFrameWidth, uiFrameHeight, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV);
	
	//pFrame = pGLScene->image2;
	//glReadPixels(0, 0, uiFrameWidth, uiFrameHeight, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, pFrame);
	//pDLVideoFrame->ser

//memset(pFrame,x, pDLVideoFrame->GetRowBytes() * uiFrameHeight);
	//pGLScene->image2->setRowLength(720);
	//memcpy(pFrame, pGLScene->image2, 720*480*2);
...
/*
	...
	bmi.biWidth = pDLVideoFrame->GetWidth();
	bmi.biHeight = pDLVideoFrame->GetHeight();
	bmi.biPlanes = 1;
	bmi.biBitCount = 32;
	...
	RECT fillRect4 = { 520, 375, 618, 475 };
	...
	*/
	...
	pDLVideoFrame->GetBytes((void**)&pbDestData);

	
	// copy the bitmap buffer content to the DeckLink frame buffer
	memcpy(pbDestData, pGLScene->image2->getDataPointer(), uiFrameWidth* uiFrameHeight*4);
	//memcpy(pbDestData, pbData, bmi.biSizeImage);
//memcpy(pbDestData, pGLScene->image2, bmi.biSizeImage);
